...
class Utils():
    ...
    def parse_string_format(self, str):
        ...
        properties_list = self.remove_duplicates(properties_list)
...
def evaluate(label_file, prediction_file):
    with open(label_file, "r", encoding="utf-8") as f:
        test_data = json.load(f)
    labels = [d["label_text"] for d in test_data]

    with open(prediction_file, "r", encoding="utf-8") as f:
        origin_predictions = json.load(f)
    
    predictions = []
    for p in origin_predictions:
        if "\n)assistant" in p:
            p = p.split("assistant")[0].strip()
        predictions.append(p)
    
    utils = Utils()
    intent_acc, element_acc, slot_f1, all_acc, acc_scores = utils.metric(labels, predictions)
...
def main():
    ...
    parser.add_argument("--label_file", type=str, required=True, help="Path to the dataset directory")
    parser.add_argument("--prediction_file", type=str, required=True, help="Path to the pretrained model directory")
    parser.add_argument("--eval_file", type=str, required=True, help="Path to the pretrained model directory")
    args = parser.parse_args()

    f1s_str = evaluate(args.label_file, args.prediction_file)
